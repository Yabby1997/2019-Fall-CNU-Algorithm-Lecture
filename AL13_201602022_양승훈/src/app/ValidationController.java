        ...
        if(this.firstFile().exists()){
            ...
            return false;
                ...
                private boolean initFirstFile() {
                    this.setFirstFilePath(AppView.inputFilePath());
                    String fileName = AppView.inputFileName();
                    String pathAndNAme = this.firstFilePath() + "/" + fileName;
                    this.setFirstFile(new File(pathAndNAme));
                    if (this.firstFile().exists()) {
                        return true;
                    } else {
                        return false;
                ...
                private boolean initFirstFile(){
                    ...
                    String pathAndName = this.firstFilePath() + "/" + name;
                    this.setFirstFile(new File(pathAndName));
                    if(this.firstFile().exists())
                        return true;
                    else
        ...
        if(this.secondFile().exists()){
            ...
            return false;
                ...
                private boolean initSecondFile(){
                    String filePath = AppView.inputAnswerForUsingSamePath() ? this.firstFilePath() : AppView.inputFilePath();
                    String fileName = AppView.inputFileName();
                    String pathAndName = filePath + "/" + fileName;
                    this.setSecondFile(new File(pathAndName));
                ...
                private boolean intSecondFile(){
                    String path = AppView.inputAnswerForUsingSamePath() ? this.firstFilePath() : AppView.inputFilePath();
                    String name = AppView.inputFileName();
    ...
    private BufferedInputStream openInputStream(File file) throws FileNotFoundException {                               //입력스트림을 연다. 파일을 인자로받는다. 비교할 두 파일의 스트림을 여는데 이용한다.
        try{
            ...
            throw e;
                ...
                final exam practice
                 */
                private BufferedInputStream openInputStream(File file) throws FileNotFoundException{
                    try{
                        FileInputStream fileInputStream = new FileInputStream(file);
                        return new BufferedInputStream(fileInputStream);
                    }
                ...
                private BufferedInputStream openInputStream(File file) throws FileNotFoundException{
                    try{
                        FileInputStream fileInputStream = new FileInputStream(file);
                        return BufferedInputStream(fileInputStream);
                    }
                    catch(FileNotFoundException e) {
                        throw e;
                    }
    ...
    private void closeInputStream(BufferedInputStream bufferedInputStream, File inputFile) throws IOException{          //입력스트림을 닫는다.
        try{
            ...
            throw e;
                ...
                final exam practice
                 */
                private void closeInputStream(BufferedInputStream bufferedInputStream) throws IOException{
                    try{
                        BufferedInputStream.close();
                    }
                    catch(IOException e){
                ...
                private void closeInputStream(BufferedInputStream stream) throws IOException{
                    try{
                        stream.close();
                    }
                    catch(IOException e){
    ...
    private boolean validate() throws IOException{
        ...
        return isValid;
                ...
                private boolean validate() throws IOException{
                    ...
                    int firstReadByte = this.readByteFromFirstInputStream();
                    int secondReadByte = this.readByteFromSecondInputStream();
                    while(firstReadByte == secondReadByte){
                        if(firstReadByte = FileIO_CONST.END_OF_INPUT_STREAM){
                            isValid = true;
                            break;
                        }
                        firstReadByte = this.readByteFromFirstInputStream();
                        secondReadByte = this.readByteFromSecondInputStream();
                    }
                    this.closeInputStream(this.firstInputStream(), this.firstFile());
                    this.closeInputStream(this.secondInputStream(), this.secondFile());
                ...
                private boolean validate() throws IOException{
                    boolean isValid = false;
                    this.setFirstInputStream(this.openInputStreeam(this.firstFile()));
                    this.setSecondInputStream(this.openInputStream(this.secondFile()));
                    int firstReadByte = this.readByteFromFirstInputStream();
                    int secondReadByte = this.readByteFromSecondInputStream();
                    while(firstReadByte == secondReadByte){
                ...
                private boolean validate() throws IOException{
                    ...
                    this.setFirstInputStream(this.openInputStream(this.firstFile()));
                    this.setSecondInputStream(this.openInputStream(this.secondFile()));
                    int firstByte = this.readByteFromFirstInputStream();
                    int secondByte = this.readByteFromSecondInputStream();
                    while(firstByte == secondByte){
                        if(firstByte == FileIO_CONST.END_OF_INPUT_STREAM) {
                            isValid = true;
                            break;
                        }
                        firstByte = this.readByteFromFirstInputStream();
                        secondByte = this.readByteFromSecondInputStream();
                    }
                    this.closeInputStream(this.firstInputStream());
                    this.closeInputSTream(this.secondInputStream());
                    return isValid;
                }
                /*
                final exam practice
