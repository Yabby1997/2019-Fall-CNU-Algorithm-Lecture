...
public final class AppView {		//입출력을 담당하는 클래
	...
	public static int inputNumberOfVertices() {
		...
		while(true) {
			...
			try {
				numberOfVertices = Integer.parseInt(scannedToken);		//scan된 scannedToken을 정수로 변환.
	...
	public static int inputSourceVertex() {								//최단경로를 찾는 경우 어디서 출발할 것인지를 입력받아야 한다. 
		int sourceVertex;
		String scannedToken;
		while(true) {
			AppView.output("? 출발 vertex 를 입력하시오 : ");
			scannedToken = AppView.scanner.next();
			try {
