...
public abstract class QuickSort<E extends Comparable<E>> extends Sort<E>{                                               //퀵소트는 피벗 선정 방식에 따라 다양한 방식으로 구현할 수 있으므로 추상클래스로 만들어준.
    public QuickSort(boolean givenSortingOrder){
        super(givenSortingOrder);
    }

    protected abstract int pivot(E[] aList, int left, int right);                                                       //해당 추상클래스를 상속받는 퀵소트는 피벗 선정방식만 다르다.
