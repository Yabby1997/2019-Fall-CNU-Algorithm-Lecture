...
public class AppController {
	...
	private static final int MIN_RECURSION_SIZE = 200;

	public AppController(){
		this.setPointSet(null);
		this.setFIndClosestPair(new FindClosestPair());
		this.setParameterSet(new ParameterSet(AppController.NUMBER_OF_STEPS, AppController.STARTING_SIZE, AppController.INCREMENT_SIZE, AppController.NUMBER_OF_REPETITIONS_OF_SAME_EXECUTION, AppController.MIN_RECURSION_SIZE));
	...
	private void showMeasurement(long[][] measurement, String measurementTitle){
		...
		AppView.outputLine(String.format("%8s %20s %20s %20s %12s", "Size", "Compare-All-Pairs", "Divide-And-Conquer", "Hybrid (MinRS : " + MIN_RECURSION_SIZE + ")", "Saving (%)"));
		...
		for(int step = 0; step < this.parameterSet().numberOfSteps(); step++){
			...
			double savingRatioByHybrid = (double) (durationByDivideAndConquer - durationByHybrid) * 100.0 / (double) durationByDivideAndConquer;
			AppView.outputLine(String.format("[%6d]%20d %20d %20d %12.0f", size, durationByComparingAllPairs, durationByDivideAndConquer, durationByHybrid, savingRatioByHybrid));
