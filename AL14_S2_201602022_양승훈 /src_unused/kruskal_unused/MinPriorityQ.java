...
public class MinPriorityQ<E extends Comparable<E>> {			//generic type으로 E를 쓸 것인데 해당 클래스 내부에서 E끼리의 비교가 발생할 수 있음을 명시한다. 제한을 둔다. 
	...
	public E removeMin() {
		if(this.isEmpty()) {
			...
			E rootElement = this.heap()[MinPriorityQ.HEAP_ROOT];	//제거될 root는 heap의 첫번째(root)요소이다. 
			...
			if(this.size() > 0) {									//제거됐을때 사이즈가 0이 아니라면, rootElement제외하고도 다른 요소들이 heap에 있으므로, 재정렬을 시켜주어야 한다. 
				...
				int parent = MinPriorityQ.HEAP_ROOT;				//root가 부모인경우부터 시작. 
